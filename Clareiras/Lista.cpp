...
NoLista* Lista::jumpToPos(int pos) {
	...
	if (pos < 0 || pos >= _numElem)
		{cout << "Erro (1) em modulo \"Lista.cpp\": impossivel saltar para posicao desejada.\n\n";	exit(1);}
...
void Lista::insere(int pos, void* info) {
	...
	if (pos < 0 || pos > _numElem) {
		cout << "Aviso (1) em modulo \"Lista.cpp\": posicao de insercao invalida. Nenhum elemento fo inserido.\n";
		return;
	}
...
void* Lista::pop() {
	...
	if (vazia())
		{cout << "Erro (2) em modulo \"Lista.cpp\": pop em lista vazia.\n\n";	exit(1);}
...
void Lista::retira(int pos) {
	...
	if (pos < 0 || pos >= _numElem) {
		cout << "Aviso (2) em modulo \"Lista.cpp\": posicao de retirada invalida. Nenhum elemento foi retirado.\n";
		return;
	}
...
void* Lista::retiraPop(int pos) {
	...
	if (pos < 0 || pos >= _numElem) {
		cout << "Erro (4) em modulo \"Lista.cpp\": posicao de retirada pop invalida.\n\n";
		exit(1);
	}
...
void*& Lista::operator[](int pos) {
	...
	if (pos < 0 || pos >= _numElem) {
		if (vazia()) 
			{cout << "Erro (3) em modulo \"Lista.cpp\": acesso em lista vazia.\n\n";	exit(1);}
		cout << "Aviso (3) em modulo \"Lista.cpp\": posicao de acesso invalida. O primeiro elemento da lista foi acessado.\n";
		pos = 0;
	}
